...
func newBranchRestrictionsCreateABranchRestrictionRuleCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if body == "" {
				...
				if bodyGroups != "" {
					handlers.SetNested(bodyObj, "groups", bodyGroups)
				}
				if bodyKind != "" {
	...
	cmd.Flags().StringVar(&bodyGroups, "groups", "", `groups (JSON array)`)
	...
	cmd.Flags().StringVar(&bodyUsers, "users", "", `users (JSON array)`)
