...
slash-separated list of path parameter values in URL order: `workspace/repo_slug/param_id`.

Using an `import` block (Terraform 1.5+):

```hcl
import {
  to = bitbucket_branch_restrictions.example
...
```shell
terraform import bitbucket_branch_restrictions.example "my-workspace/my-repo/1"
```
