...
slash-separated list of path parameter values in URL order: `workspace/repo_slug/pull_request_id/comment_id`.

Using an `import` block (Terraform 1.5+):

```hcl
import {
  to = bitbucket_pr_comments.example
...
```shell
terraform import bitbucket_pr_comments.example "my-workspace/my-repo/1/1"
```
