...
slash-separated list of path parameter values in URL order: `workspace/repo_slug/issue_id`.

Using an `import` block (Terraform 1.5+):

```hcl
import {
  to = bitbucket_issues.example
...
```shell
terraform import bitbucket_issues.example "my-workspace/my-repo/1"
```
