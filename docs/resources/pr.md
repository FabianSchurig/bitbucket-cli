...
- `reviewers` (String) The list of users that were added as reviewers on this pull request when it was created. For performance reasons, the API only includes this list on a pull request's `self` URL. (JSON array) (also computed from API response)
...
- `participants` (String) The list of users that are collaborating on this pull request.
