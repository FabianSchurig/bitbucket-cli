...
slash-separated list of path parameter values in URL order: `workspace/repo_slug/uid`.

Using an `import` block (Terraform 1.5+):

```hcl
import {
  to = bitbucket_hooks.example
  id = "my-workspace/my-repo/webhook-uuid"
}
```
...
```shell
terraform import bitbucket_hooks.example "my-workspace/my-repo/webhook-uuid"
```
