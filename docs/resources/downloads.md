...
slash-separated list of path parameter values in URL order: `workspace/repo_slug/filename`.

Using an `import` block (Terraform 1.5+):

```hcl
import {
  to = bitbucket_downloads.example
  id = "my-workspace/my-repo/artifact.zip"
}
```
...
```shell
terraform import bitbucket_downloads.example "my-workspace/my-repo/artifact.zip"
```
